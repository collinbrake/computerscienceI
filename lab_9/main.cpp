...
#include <sstream>
#include <limits>
#include <cstdio> // to get error handling tools
...
int main()
{
  ...
  {
    errorHandler("Salary file", 2, "Input file does not exist.");
  }
  salaryFile.close();
  newSalaryFile.close();
  return 0;
...
void adjustSalary(ifstream &salaryFile, ofstream &newSalaryFile)
{
  ...
  while (!(salaryFile >> stringAmount).eof())        // avoids the isolated eof test from failure (as explained in our lectures)
  {
    ...
    {
      errorHandler(stringAmount, 1, "Non-numeric (invalid) quantity.");
    }
    else if (salaryFile.fail() && !salaryFile.eof())
    {
      ...
      if (salaryFile.bad())
      {
         errorHandler("Salary file", 2, "Data stream unusable.");
      }
      else
      {
         errorHandler("Salary file", 1, "Bad data, clearing buffer.");
    ...
    {
      errorHandler("Salary amount", 1, "Invalid (<= 0) quantity.");
    }
    else if (amount > 999999.99)                   // invalid amount error over a million (status ONE)
    {
      errorHandler("Salary amount", 1, "Invalid (>= 1 million) quantity.");
    }
    else                                           // trailing else ... everything was valid.
    {
      ...
      {
         errorHandler("salary file", 2, "Failed to write to file.");
      }
      validCount++;
    }
  }
...
void errorHandler(string dataInfo, int status, string message)
{
   ...
   {
      case 1:
         cerr << "Error status " << status << ", data info: "
            << dataInfo << ", message: " << message << endl;
      case 2:
         cerr << "Error status " << status << ", data info: "
            << dataInfo << ", message: " << message << endl;
   }
